...
def back(x,y):#배경이미지를 게임판 위에 그려주는 함수 배경이미지가 game판과 크기가 같으니 x,y값은 각각 0,0이 되어야 게임판에 빈틈없이 채워집니다. 
...
def runGame(): #실제 게임이 구동되는 함수 밑에 initGame()함수에서 호출됨
    ...
    background_x=0#배경이미지의 좌상단 모서리의 x좌표를 나타내고 최초값으로 0지정
    ...
    while not crashed:
        ...
        back(background_x,0)# 배경이미지를 게임판에 그리기 위해 좌상단 모서리의 x좌표와 y좌표를 인자로 전달, 최초값은 0,0
        airplane(x,y)
...
def initGame():#게임을 초기화하고 시작하는 함수
    ...
    background = pygame.image.load('images/background.png')#배경이미지 파일을 읽어 전역변수 background에 할
